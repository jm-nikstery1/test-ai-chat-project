...
def populate_neo4j(driver, game_data):
    ...
    with driver.session() as session:
        ...
        for game in tqdm(game_data, desc="Neo4j 데이터 저장 중"):
            ...
            # Developer, Publisher, Genre, Category, Tag 노드 및 관계 생성
            ...
            for tag in split_string_to_list(game.get('tags', [])):
                session.run("""
                    MERGE (t:Tag {name: $tag_name})
                    WITH t
                    MATCH (g:Game {appid: $appid})
                    MERGE (g)-[:HAS_TAG]->(t)
                """, tag_name=tag, appid=game.get('appid'))
...
def populate_qdrant(client, embedding_model, game_data):
    ...
    for game in tqdm(game_data, desc="Qdrant 데이터 임베딩 및 준비 중"):
        ...
        semantic_text = (
            ...
            f"Tags: {', '.join(split_string_to_list(game.get('tags', [])))}. "
        ...
        payload = {
            ...
            "tags": split_string_to_list(game.get('tags', [])),
